...
def lstmmodel(filter0, filter1, rate0, rate1):
    model = Sequential()
    model.add(LSTM(units = filter0, input_shape = (n_steps_in, n_features), return_sequences = True))
    model.add(BatchNormalization())
    model.add(Dropout(rate = rate0))  
    model.add(LSTM(units = filter1, input_shape = (n_steps_in, n_features), return_sequences = False))
    model.add(BatchNormalization())
